...
ENV VIRTUAL_ENV /opt/ngcp-logic-engine-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/ngcp-logic-engine/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/ngcp-logic-engine/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/ngcp_logic_engine/ && touch src/ngcp_logic_engine/__init__.py && touch README.md
...
RUN --mount=type=cache,target=/var/cache/apt/ \
    --mount=type=cache,target=/var/lib/apt/ \
    apt-get update && \
    apt-get install --no-install-recommends --yes curl git gnupg ssh sudo vim zsh git-review && \
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/ngcp-logic-engine/
RUN mkdir -p /opt/build/poetry/ && cp poetry.lock /opt/build/poetry/ && \
    git init && pre-commit install --install-hooks && \
    mkdir -p /opt/build/git/ && cp .git/hooks/pre-commit /opt/build/git/
...
# Expose the app.
ENTRYPOINT ["/opt/ngcp-logic-engine-env/bin/poe"]
CMD ["api"]
